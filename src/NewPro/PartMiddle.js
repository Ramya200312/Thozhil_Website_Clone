...
function PartMiddle({ data1 }) {
    ...
    const renderRightSideCards1 = () => {
        // Calculate the starting and ending index for the current page
        const startIndex = (currentPage1 - 1) * itemsPerPage;
        const endIndex = startIndex + itemsPerPage;

        // Slice the data array to get the items for the current page
        const currentItems = data1.slice(startIndex, endIndex);
    ...
    const renderPagination = () => {
        ...
        for (let i = 1; i <= totalPages; i++) {
            pageNumbers.push(
                <Button
                    key={i}
                    variant={i === currentPage1 ? 'contained' : 'outlined'}
                    onClick={() => handlePageChange1(i)}
                    sx={{ mx: 0.5 }}
                >
                    {i}
                </Button>
